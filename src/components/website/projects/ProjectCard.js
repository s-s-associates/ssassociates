...
import { Box, Button, Grid, MenuItem, Select, Skeleton, Typography } from "@mui/material";
...
export default function ProjectCard({ maxProjects, showCategoryFilter } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const mapped = data.projects.map(mapProjectToCard);
          setAllProjects(maxProjects ? mapped.slice(0, maxProjects) : mapped);
        }
      } catch (e) {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await res.json();
        if (data.success && Array.isArray(data.categories) && !cancelled) {
