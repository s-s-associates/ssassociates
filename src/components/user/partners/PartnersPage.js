...
export default function PartnersPage() {
  ...
  const filteredPartners = partners.filter((row) => {
  ...
  const handleSave = async () => {
    ...
    const hasImage =
      ...
      (editingPartner?.imageUrl && imagePreview === editingPartner.imageUrl);
    if (!hasImage) {
      setImageError(true);
      await Swal.fire({
        icon: "warning",
        title: "Image required",
        text: "Please upload an image for this partner.",
        confirmButtonColor: primaryColor,
      });
      return;
    }
    ...
    try {
      ...
      } else if (editingPartner?.imageUrl) {
  ...
  const handleDelete = (partner) => {
    Swal.fire({
      ...
      text: `"${partner.title}" will be removed.`,
      icon: "warning",
    ...
    <Box sx={{ p: { xs: 2, sm: 3 }, mx: "auto", bgcolor: bggrayColor, minHeight: "100vh" }}>
      ...
      <Dialog open={dialogOpen} onClose={closeDialog} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          >
            {saving ? <BeatLoader color="#fff" size={12} /> : editingPartner ? "Update" : "Add"}
          </Button>
        </DialogActions>
      </Dialog>
          ...
          {viewingPartner && (
            <Box>
              {viewingPartner.imageUrl ? (
                <Box
                  component="img"
                  src={viewingPartner.imageUrl}
                  alt={viewingPartner.title}
                  loading="lazy"
                  sx={{
                    width: "100%",
                    maxHeight: 280,
              ...
              {viewingPartner.url ? (
                <>
                  ...
                  <Box
                    component="a"
                    href={viewingPartner.url.startsWith("http") ? viewingPartner.url : `https://${viewingPartner.url}`}
                    target="_blank"
